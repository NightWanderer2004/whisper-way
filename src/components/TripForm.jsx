...
export default function TripForm({ isLoading, setIsLoading, setLocations }) {
   const { setUserData, setMainCityCoords, updateTripData, setShowMap } = useTripStore()
   ...
   const onSubmit = async formData => {
      ...
      try {
         ...
         const toiletsPromises =
            tripData.public_toilets?.map(toilet =>
               getCoordinates(toilet, mainCityInfo).catch(error => {
                  console.warn(`Failed to get coordinates for ${toilet.name}:`, error)
                  return null
               }),
            ) || []

         const [resolvedLocations, resolvedToilets] = await Promise.all([Promise.all(locationPromises), Promise.all(toiletsPromises)])

         const formattedLocations = resolvedLocations.filter(Boolean)
         const formattedToilets = resolvedToilets.filter(Boolean)

         if (formattedLocations.length === 0 && formattedToilets.length === 0) {
         ...
         const updatedTripData = {
            ...
            public_toilets: formattedToilets,
         }
